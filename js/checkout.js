document.addEventListener("DOMContentLoaded", () => {
  ...
  function calculate() {
    ...
    const promotionItems = valid.map((row) => ({ product: row.product, quantity: row.entry.quantity }));
    ...
    const method = currentDelivery();
    const promotionCalculation = window.TabanjiPromotions.calculateForDelivery(promotionItems, method);
    return { ...promotionCalculation, productDiscount, promoCode: promotionCalculation.applied.map((item) => item.code).join(", "), deliveryPrice: Math.max(0, promotionCalculation.shipping - promotionCalculation.shippingDiscount), currency, mixed: currencies.size > 1, promotionCalculation };
  }
